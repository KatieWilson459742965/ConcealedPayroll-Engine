...
import { toastTxPending, toastTxSuccess, toastTxError, toastUserRejected } from '@/lib/toast-utils';
...
export const usePayroll = () => {
  ...
  const createOrganization = useCallback(async (
    ...
    try {
      ...
      if (receipt.status === 1) {
        toastTxSuccess(txHash, 'Organization created successfully!');
  ...
  const addTeamMember = useCallback(async (
    ...
    try {
      ...
      const { handle: salaryHandle, proof: salaryProof } = await encryptSalary(
      ...
      if (receipt.status === 1) {
        toastTxSuccess(txHash, `${memberName} added successfully!`);
      } else {
        throw new Error('Transaction failed');
      }
    } catch (error: any) {
  ...
  const createPayrollDistribution = useCallback(async (
    ...
    try {
      ...
      const { handle: periodHandle, proof: periodProof } = await encryptUint32(
        periodNum,
        CONTRACT_ADDRESS,
        address
      );
      ...
      if (receipt.status === 1) {
        toastTxSuccess(txHash, 'Payroll distribution created successfully!');
  ...
  const executeDistribution = useCallback(async (distributionId: string) => {
    ...
    try {
      ...
      if (receipt.status === 1) {
        toastTxSuccess(txHash, 'Payroll distribution executed!');
      } else {
        throw new Error('Transaction failed');
      }
    } catch (error: any) {
